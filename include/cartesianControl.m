...
classdef cartesianControl < handle
    ...
    methods
        ...
        function [x_dot]=getCartesianReference(self,bTg)
            ...
            current_tool_position = bTt(1:3, 4); % current tool position 
            current_tool_orientation = bTt(1:3, 1:3); % current tool orientation

            bOg = bTg(1:3, 4); % goal position
            bRg = bTg(1:3, 1:3); % goal orientation

            % linear goal error
            lin_err = bOg - current_tool_position;
            v_c = self.k_l * lin_err;

            % angular goal error
            c = bRg * current_tool_orientation';
            skew_err = (c - c')/2;
            ang_err = [ skew_err(3,2);
                        skew_err(1,3);
                        skew_err(2,1)];
            omega_c = self.k_a*ang_err; % omega control 
